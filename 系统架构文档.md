...
> 版本 1.3.0 | 技术栈：Electron 28 + Vue 3 + TypeScript + SQLite

> **v1.3.0 更新摘要**：新增课程1「部署自己的 Agent」（4 章节）+ 6 道 guide 模式实践题；validationType 新增 guide 纯引导类型；现有课程/任务顺延；总计 7 课程 / 19 章节 / 12 任务（6 guide + 6 tests）。详见第十七章。
>
> **v1.2.2 更新摘要**：修复章节视频不显示的前端字段映射 bug — CoursesStore.loadChapters 未将数据库 `video_url` 下划线字段映射为前端 Chapter 接口的 `videoUrl` 驼峰字段，导致 B 站教学视频区域始终不渲染
>
...
| 代码执行沙箱 | Web Worker | 原生 | 🆕 v1.3.0 guide 类型不经过沙箱 |
...
│   ├── database.ts              # SQLite 数据库初始化 + 种子数据  🆕 v1.2 chapters +resources 列 / 19 章真实 B 站视频 / 12 任务步骤扩展 referenceCode+expectedResult；🆕 v1.3.0 新增课程1 部署 Agent（4 章）+ 6 道 guide 实践题，data_version=1.3.0
...
>
> 🆕 **v1.3.0 数据库迁移**：新增课程 1「部署自己的 Agent」（4 章节）+ 6 道 guide 类型实践题；courses 6→7、chapters 15→19、tasks 6→12；`app_meta.data_version` 由 1.2.1 升至 1.3.0。升级时版本号不匹配会清除旧课程/章节/任务数据并重新填充（机制同 7.4 节）。
...
> 表中 `_v11` 标记为 v1.1 新增列，`_v12` 标记为 v1.2 新增列。**v1.3.0 数据量**：courses 7 条 / chapters 19 条 / tasks 12 条。
...
| 表 | 记录数 | 说明 | 变更 |
|----|--------|------|------|
| courses | 7 | 部署自己的 Agent、Agent 基础、Prompt、Tool Calling、工作流、多 Agent、RAG | 🆕 v1.3.0 新增课程 1 |
| chapters | 19 | 理论 10 章 + 实操 9 章（含部署 Agent 4 章），每章含 Markdown 内容 | 🆕 v1.2 全部补充真实 B 站 BV 号 + 4 个 resources；🆕 v1.3.0 新增 4 章 |
| tasks | 12 | 6 道 guide 实践题 + 5 个核心任务 + 1 个综合实战引导项目 | v1.1 含 steps/test_cases/function_name；🆕 v1.2 steps 扩展 referenceCode/expectedResult；🆕 v1.3.0 新增 6 道 guide 任务 |
| user_config | 1 | 默认昵称"学习者"，目标 30 分钟/天 | v1.1 含 LLM 默认配置 |
| app_meta | 1 | `data_version = 1.3.0` | 🆕 v1.2.1 新增；🆕 v1.3.0 版本升至 1.3.0 |
...
| `ValidationType` | 验证类型：exact / contains / regex / custom / tests / guide | v1.1 新增 `tests`；🆕 v1.3.0 新增 `guide` |
...
## 十六、v1.2.2 变更日志（视频字段映射修复）

### 16.1 问题背景

v1.2.1 修复了数据库迁移后，学习资源和任务引导正确显示，但**章节视频仍然不显示**。

### 16.2 根本原因

[src/stores/courses.ts](file:///d:/ai/6a5dd856e73672a131495d9d/src/stores/courses.ts) 的 `loadChapters` 函数在映射数据库行到 Chapter 对象时，只解析了 `resources` JSON 字段，**未将数据库下划线字段 `video_url` 映射为前端 Chapter 接口期望的驼峰字段 `videoUrl`**。

```typescript
// v1.2.1 的有 bug 代码
const parsed = rows.map((row: any) => ({
  ...row,  // ← row.video_url 保留，但 Chapter 接口需要 videoUrl
  resources: row.resources ? (JSON.parse(row.resources) as ChapterResource[]) : null
})) as Chapter[]
```

[ChapterView.vue](file:///d:/ai/6a5dd856e73672a131495d9d/src/views/ChapterView.vue) 的 `videoEmbedUrl` computed 读取 `chapter.videoUrl`（驼峰），但数据库返回的是 `video_url`（下划线），所以始终为 `undefined`，视频区域永远不渲染。

```mermaid
flowchart LR
    A[数据库 chapters 表] -->|video_url=BV1Xi...| B[loadChapters]
    B -->|...row 展开| C[row.video_url 有值<br/>row.videoUrl = undefined]
    C --> D[ChapterView 读取 chapter.videoUrl]
    D --> E{videoUrl 有值?}
    E -->|undefined| F[视频区域不渲染 ❌]
...
在 `loadChapters` 中添加 `videoUrl: row.video_url` 字段映射：
...
flowchart LR
    A[数据库 chapters 表] -->|video_url=BV1Xi...| B[loadChapters]
    B -->|显式映射 videoUrl| C[row.video_url 有值<br/>videoUrl = row.video_url ✅]
    C --> D[ChapterView 读取 chapter.videoUrl]
    D --> E{videoUrl 有值?}
    E -->|BV1Xi...| F[videoEmbedUrl 提取 BV 号]
    F --> G[渲染 Bilibili iframe ✅]

    style C fill:#5B8C5A,color:#fff
...
| 文件 | 变更 |
|------|------|
| `src/stores/courses.ts` | loadChapters 添加 `videoUrl: row.video_url ?? null` 字段映射 |
| `package.json` | 版本号 1.2.1 → 1.2.2 |
| `src/views/SettingsView.vue` | 关于信息 v1.2.1 → v1.2.2 |
| `README.md` | 新增 v1.2.2 更新说明 |
| `使用文档.md` | 版本号同步 |
| `系统架构文档.md` | 新增第十六章 |

### 16.5 为什么 resources 能显示但视频不能

| 字段 | 数据库列名 | 前端接口字段 | loadChapters 处理 | 结果 |
|------|-----------|-------------|-------------------|------|
| 资源 | `resources` | `resources` | ✅ 显式 `JSON.parse(row.resources)` | 正常显示 |
| 视频 | `video_url` | `videoUrl` | ❌ 未映射，仅 `...row` 保留 `video_url` | 不显示 |

> 注意：`...row` 展开保留的是数据库原始列名 `video_url`，而 ChapterView.vue 读取的是驼峰 `videoUrl`，两者不匹配导致视频始终为 undefined。

### 16.6 不需要数据库迁移

此修复纯前端字段映射问题，数据库中的 `video_url` 数据在 v1.2.1 迁移时已正确写入。因此 v1.2.2 **不更新 `CURRENT_DATA_VERSION`**，从 v1.2.1 升级时不会触发数据库重置，用户学习时长统计完全保留。

---

## 十七、v1.3.0 变更日志（部署 Agent 课程 + guide 实践题）

### 17.1 变更总览

| 模块 | 变更类型 | 说明 |
|------|----------|------|
| 课程学习 | 新增课程 | 新增课程 1「部署自己的 Agent」（4 章节），现有 6 门课程顺延为课程 2-7 |
| 实操实验室 | 新增任务 | 新增 6 道 guide 模式实践题（任务 1-6），现有 6 道任务顺延为任务 7-12 |
| 实操实验室 | 新增验证类型 | validationType 新增 `guide` 纯引导类型，无测试用例和代码运行，用户手动标记完成 |
| 类型系统 | 扩展 | ValidationType 新增 `guide` |
| 数据库 | 种子数据 | courses 6→7、chapters 15→19、tasks 6→12；data_version 升至 1.3.0 |

### 17.2 新增课程 1：部署自己的 Agent

| 章节 | 类型 | 内容 |
|------|------|------|
| 环境准备与 API Key 配置 | 实操 | Node.js/Python 环境准备、OpenAI 兼容 API Key 申请与配置 |
| Python + LangChain 部署 Agent | 实操 | 使用 LangChain 框架快速部署一个可对话的 Agent |
| Node.js 部署 Agent | 实操 | 使用 Node.js + OpenAI SDK 部署流式输出 Agent |
| 本地部署开源模型（Ollama） | 实操 | 安装 Ollama、拉取开源模型、部署本地离线 Agent |

### 17.3 新增 guide 验证类型

`ValidationType` 新增 `guide` 类型，特点：
- **纯引导模式**：仅提供题目描述和分步引导（steps），不包含测试用例（testCases）
- **无在线运行**：不经过 Web Worker 沙箱执行代码，无「运行测试」按钮
- **手动完成**：用户在自己的 Agent 环境中完成实践后，手动点击「标记为已完成」

```mermaid
flowchart LR
    A[任务 validationType] --> B{类型判断}
    B -->|tests| C["Monaco 编辑器 + Web Worker 沙箱 + 单元测试"]
    B -->|guide| D["题目描述 + 分步引导 + 手动标记完成"]
    C --> E["全部测试通过 → 完成"]
    D --> F["用户自行实践 → 手动标记完成"]
    ...
    style D fill:#5B8C5A,color:#fff
    style E fill:#5B8C5A,color:#fff
    style F fill:#5B8C5A,color:#fff
```

### 17.4 新增 6 道 guide 实践题

| 编号 | 任务 | 说明 |
|------|------|------|
| 1 | 获取并配置 API Key | 申请并配置 LLM API Key |
| 2 | Python LangChain Agent | 用 LangChain 部署 Agent |
| 3 | Agent 工具调用 | 为 Agent 注册并调用工具 |
| 4 | Node.js 流式输出 | Node.js 流式响应 Agent |
| 5 | Ollama 本地 Agent | 基于 Ollama 的本地 Agent |
| 6 | 综合实战：多轮对话 Agent | 引导项目，多轮对话综合实战 |

### 17.5 数据库变更

| 表 | 变更 |
|------|------|
| `courses` | 新增课程 1「部署自己的 Agent」（4 章节），现有课程 sort_order 顺延 |
| `chapters` | 新增 4 章节（部署 Agent 相关），总章节数 15→19 |
| `tasks` | 新增 6 道 guide 类型任务，总任务数 6→12 |
| `app_meta` | `data_version` 由 1.2.1 升至 1.3.0 |

> v1.3.0 通过版本号驱动迁移（详见 7.4 节机制）：当 `data_version` 不等于 1.3.0 时，清除旧 courses/chapters/tasks 数据并重新填充 7 课程 / 19 章节 / 12 任务，写入 `data_version = 1.3.0`。

### 17.6 文件变更清单

| 文件 | 主要变更 |
|------|----------|
| `electron/database.ts` | 新增课程 1（4 章节）+ 6 道 guide 任务；data_version 升至 1.3.0 |
| `src/types/index.ts` | ValidationType 新增 `guide` 类型 |
| `package.json` | 版本号 1.2.2 → 1.3.0 |
| `src/views/SettingsView.vue` | 关于信息 1.2.2 → 1.3.0 |
| `README.md` | 新增 v1.3.0 更新说明 |
| `使用文档.md` | 版本号同步、新增课程与 guide 任务说明 |
| `系统架构文档.md` | 新增第十七章；更新技术栈/数据库/类型说明 |