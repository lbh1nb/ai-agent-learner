...
> 推荐进一步阅读：[Anthropic - Building effective agents](https://www.anthropic.com/research/building-effective-agents)`, 'theory', 15, 'https://www.bilibili.com/video/BV19tZPYpE1S/', 1)
...
> 推荐阅读：[Lilian Weng - LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/)`, 'theory', 15, 'https://www.bilibili.com/video/BV19tZPYpE1S/?p=2', 2)
...
> 推荐阅读：[OpenAI - Building agents with the OpenAI API](https://platform.openai.com/docs/guides/agents)`, 'theory', 15, 'https://www.bilibili.com/video/BV19tZPYpE1S/?p=3', 3)
...
> 推荐阅读：[OpenAI - Prompt engineering guide](https://platform.openai.com/docs/guides/prompt-engineering)`, 'theory', 15, 'https://www.promptingguide.ai/zh/introduction/tips', 1)
...
> 推荐阅读：[Prompt Engineering Guide - 高级技巧](https://www.promptingguide.ai/zh/techniques/cot)`, 'theory', 15, 'https://www.promptingguide.ai/zh/techniques/cot', 2)
...
> 推荐阅读：[Anthropic - Prompt engineering with Claude](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview)`, 'practice', 20, 'https://www.promptingguide.ai/zh/examples', 3)
...
> 推荐阅读：[OpenAI - Function calling guide](https://platform.openai.com/docs/guides/function-calling)`, 'theory', 15, 'https://platform.openai.com/docs/guides/function-calling', 1)
...
> 推荐阅读：[LangChain - Tools and ToolCalling](https://python.langchain.com/docs/modules/agents/tools/)`, 'theory', 15, 'https://python.langchain.com/docs/modules/agents/tools/', 2)
...
> 推荐阅读：[LangChain - Custom Tools](https://python.langchain.com/docs/modules/agents/tools/custom_tools)`, 'practice', 20, 'https://python.langchain.com/docs/modules/agents/tools/custom_tools', 3)
...
> 推荐阅读：[Anthropic - Building effective agents](https://www.anthropic.com/research/building-effective-agents)`, 'theory', 15, 'https://www.anthropic.com/research/building-effective-agents', 1)
...
> 推荐阅读：[LangGraph - Build Stateful Agents](https://langchain-ai.github.io/langgraph/)`, 'practice', 20, 'https://langchain-ai.github.io/langgraph/', 2)
...
> 推荐阅读：[AutoGen - Multi-agent framework](https://microsoft.github.io/autogen/)`, 'theory', 15, 'https://microsoft.github.io/autogen/', 1)
...
> 推荐阅读：[CrewAI - Multi-agent framework](https://docs.crewai.com/)`, 'practice', 20, 'https://docs.crewai.com/', 2)
...
> 推荐阅读：[Pinecone - What is RAG?](https://www.pinecone.io/learn/retrieval-augmented-generation/)`, 'theory', 15, 'https://www.pinecone.io/learn/retrieval-augmented-generation/', 1)
...
> 推荐阅读：[LangChain - RAG Tutorial](https://python.langchain.com/docs/tutorials/rag)`, 'practice', 20, 'https://python.langchain.com/docs/tutorials/rag', 2)
    ...
    insertTask.run(
      ...
      'tests', '', 'easy', 1,
      JSON.stringify([
        { title: '定义函数签名', description: '创建 `createAgentPrompt(role, task, constraints)` 函数，接收三个参数', hint: '使用 `function createAgentPrompt(role, task, constraints) { }` 定义函数' },
        { title: '拼接角色信息', description: '在返回的字符串中包含 role 参数', hint: '使用模板字符串 `${role}` 嵌入角色名' },
        { title: '拼接任务描述', description: '在返回的字符串中包含 task 参数', hint: '继续使用模板字符串拼接 task' },
    ...
    insertTask.run(
      ...
      'tests', '', 'medium', 1,
      JSON.stringify([
        { title: '创建内部存储', description: '在函数内部创建一个 Map 或对象，用于存储工具', hint: '使用 `const registry = new Map()` 创建映射' },
        { title: '注册工具', description: '遍历 tools 数组，将每个工具按 name 存入 registry', hint: '使用 `tools.forEach(t => registry.set(t.name, t))` 或 for 循环' },
        { title: '实现 definitions 操作', description: '当 action 为 \'definitions\' 时，返回所有工具的定义（不含 handler）', hint: '使用 `Array.from(registry.values()).map(t => ({...}))` 提取并裁剪字段' },
        { title: '实现 execute 操作', description: '当 action 为 \'execute\' 时，查找并调用对应工具的 handler', hint: '使用 `registry.get(target)` 查找工具，然后 `tool.handler(params)` 执行' }
      ]),
      JSON.stringify([
    ...
    insertTask.run(
      ...
      'tests', '', 'hard', 1,
      JSON.stringify([
        { title: 'Plan 阶段', description: '提取 steps 中所有步骤的 name，组成 plan 数组', hint: '使用 `steps.map(s => s.name)` 提取名称' },
        { title: 'Execute 阶段', description: '依次调用每个步骤的 handler，收集结果到 results 数组', hint: '使用 `steps.map(s => s.handler())` 执行所有 handler' },
        { title: 'Review 阶段', description: '生成固定格式的 review 字符串', hint: '模板字符串：`工作流完成：${steps.length} 个步骤全部成功`' },
        { title: '组装返回值', description: '返回 {plan, results, review} 对象', hint: '直接 `return { plan, results, review }`' }
      ]),
      JSON.stringify([
    ...
    insertTask.run(
      ...
      'tests', '', 'hard', 1,
      JSON.stringify([
        { title: '过滤目标消息', description: '从 messages 中筛选 to === agentId 的消息', hint: '使用 `messages.filter(m => m.to === agentId)` 过滤' },
        { title: '裁剪字段', description: '每条消息只保留 from 和 content 两个字段', hint: '使用 `.map(m => ({ from: m.from, content: m.content }))` 重组' },
        { title: '保持顺序', description: '返回的消息顺序应与原数组中的顺序一致', hint: 'filter 和 map 不会改变顺序，链式调用即可' }
      ]),
      JSON.stringify([
    ...
    insertTask.run(
      ...
      'tests', '', 'hard', 1,
      JSON.stringify([
        { title: '查询分词', description: '将 query 字符串按空格拆分为单词集合', hint: '使用 `new Set(query.split(/\\s+/))` 创建单词集合' },
        { title: '计算相似度', description: '对每个文档，统计与 query 共享的单词数量', hint: '遍历文档单词，检查是否在 queryWords 中：`if (queryWords.has(w)) score++`' },
        { title: '排序', description: '按相似度降序排序，相似度相同的保持原顺序', hint: '使用 `scored.sort((a, b) => b.score - a.score)` 降序排列' },
        { title: '截取 topK', description: '取前 topK 个文档返回', hint: '使用 `scored.slice(0, topK).map(s => s.doc)` 截取并提取文档' }
      ]),
      JSON.stringify([
    ...
    insertTask.run(
      ...
      'tests', '', 'hard', 2,
      JSON.stringify([
        { title: '实现检索函数', description: '在 runAgent 内部定义 retrieve(query, docs, topK) 函数，基于关键词匹配检索文档', hint: '参考任务5：用 Set 存储单词，统计共享词数，按相似度排序取 topK' },
        { title: '过滤零相似度文档', description: '检索结果应排除相似度为 0 的文档（无任何共享单词）', hint: '在排序后用 `scored.filter(s => s.score > 0)` 过滤' },
        { title: '实现回答生成', description: '定义 generateAnswer(retrievedDocs, question) 函数：有文档时返回"根据知识库："+第一条；无文档时返回"抱歉..."', hint: '使用 if-else 判断 retrievedDocs.length 是否为 0' },
        { title: '组合 Agent 流程', description: '调用 retrieve 检索，再调用 generateAnswer 生成回答，返回最终结果', hint: 'const retrieved = retrieve(question, documents, 1); return generateAnswer(retrieved, question)' }
      ]),
      JSON.stringify([
