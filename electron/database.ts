...
function createTables(): void {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS tasks (
      ...
      initial_code TEXT,
      solution_code TEXT,
      validation_type TEXT CHECK(validation_type IN ('exact','contains','regex','custom','tests','guide')) DEFAULT 'contains',
...
function seedData(): void {
  ...
  const seed = db.transaction(() => {
    ...
    // 课程 1: 部署自己的 Agent
    insertCourse.run(1, '部署自己的 Agent', 'basics', 'beginner', '从零开始部署你的第一个 AI Agent，覆盖 Python、Node.js、API Key 配置和本地开源模型', 1)

    // 章节 1: 环境准备与 API Key 配置
    insertChapter.run(1, 1, '环境准备与 API Key 配置', `# 环境准备与 API Key 配置

在开始部署 AI Agent 之前，你需要准备好开发环境并获取 API Key。本章将手把手教你完成所有准备工作。

## 1. 安装 Python 开发环境

Python 是部署 AI Agent 最常用的语言，LangChain、LlamaIndex 等主流框架都原生支持 Python。

### Windows 安装步骤

1. 访问 [Python 官网](https://www.python.org/downloads/) 下载最新版 Python（建议 3.10 以上）
...
Node.js 是另一种常用的 Agent 开发环境，特别适合 Web 应用场景。
...
2. 注册账号
3. 在 API Keys 页面创建密钥
4. DeepSeek 兼容 OpenAI API 格式，价格更实惠

### 通义千问 API Key

1. 访问 [阿里云 DashScope](https://help.aliyun.com/zh/dashscope/)
2. 开通 DashScope 服务
3. 在控制台获取 API Key
...
**永远不要把 API Key 硬编码在代码里！** 正确做法是使用环境变量。
...
如果看到回复，恭喜你！环境准备完成，可以开始部署 Agent 了。`, 'theory', 20, 'https://www.bilibili.com/video/BV1XiTkzVE4X/', 1, JSON.stringify([
    {title: 'OpenAI API 文档', url: 'https://platform.openai.com/docs'},
    {title: 'DeepSeek 开放平台', url: 'https://platform.deepseek.com/'},
    {title: '通义千问 API', url: 'https://help.aliyun.com/zh/dashscope/'},
    {title: 'Moonshot Kimi API', url: 'https://platform.moonshot.cn/'}
  ]))
    // 章节 2: 使用 Python + LangChain 部署 Agent
    insertChapter.run(2, 1, '使用 Python + LangChain 部署 Agent', `# 使用 Python + LangChain 部署 Agent

LangChain 是目前最流行的 AI Agent 开发框架之一。本章将教你用 Python + LangChain 从零创建你的第一个 Agent。
...
result = executor.invoke({"input": "请计算 123 * 456"})
print(result["output"])
\`\`\`
...
现在你已经可以用 Python + LangChain 部署一个功能完整的 Agent 了！`, 'theory', 25, 'https://www.bilibili.com/video/BV1Z7ZwYHENT/', 2, JSON.stringify([
    {title: 'LangChain 官方文档', url: 'https://python.langchain.com/docs/get_started/introduction'},
    {title: 'LangChain Agent 教程', url: 'https://python.langchain.com/docs/modules/agents/'},
    {title: 'OpenAI Python SDK', url: 'https://github.com/openai/openai-python'},
    {title: 'Python 官方下载', url: 'https://www.python.org/downloads/'}
  ]))
    // 章节 3: 使用 Node.js 部署 Agent
    insertChapter.run(3, 1, '使用 Node.js 部署 Agent', `# 使用 Node.js 部署 Agent

如果你更熟悉 JavaScript/TypeScript，Node.js 是部署 Agent 的绝佳选择。本章教你用 Node.js + OpenAI SDK 创建 Agent。

## 1. 初始化项目

\`\`\`bash
mkdir my-node-agent && cd my-node-agent
npm init -y
...
\`\`\`json
{ "type": "module", "dependencies": { "openai": "^4.0.0", "dotenv": "^16.0.0" } }
\`\`\`

## 2. 配置环境变量

创建 \`.env\` 文件：\`OPENAI_API_KEY=sk-你的密钥\`
...
function calculate(expression) {
  try { return String(eval(expression)) } catch (e) { return '计算失败' }
}
...
async function runAgent(userInput) {
  ...
  if (message.tool_calls) {
    ...
    const finalResponse = await client.chat.completions.create({ model: 'gpt-4o-mini', messages, tools })
    return finalResponse.choices[0].message.content
  }
  return message.content
}
console.log(await runAgent('请计算 123 * 456'))
\`\`\`

现在你可以用 Node.js 部署功能完整的 Agent 了！`, 'theory', 20, 'https://www.bilibili.com/video/BV1z95LzaE39/', 3, JSON.stringify([
    {title: 'Node.js 官方文档', url: 'https://nodejs.org/docs/latest/api/'},
    {title: 'OpenAI Node SDK', url: 'https://github.com/openai/openai-node'},
    {title: 'LangChain.js 文档', url: 'https://js.langchain.com/docs/'},
    {title: 'npm 文档', url: 'https://docs.npmjs.com/'}
  ]))
    // 章节 4: 本地部署开源模型（Ollama）
    insertChapter.run(4, 1, '本地部署开源模型（Ollama）', `# 本地部署开源模型（Ollama）

不想花钱买 API？Ollama 让你在本地电脑上运行开源大模型，完全免费、数据私密！
...
Ollama 支持多种开源模型，推荐以下几种：

\`\`\`bash
# 拉取 Qwen 2.5（阿里通义千问，中文能力强）
ollama pull qwen2.5:7b

# 拉取 Llama 3.2（Meta 开源模型）
...
输入问题即可开始对话，输入 \`/bye\` 退出。

## 3. Ollama API 端点

Ollama 启动后会自动在 \`http://localhost:11434\` 提供 API 服务，兼容 OpenAI API 格式。
...
| 模型 | 大小 | 最低内存 | 适合场景 |
|------|------|---------|---------|
| qwen2.5:1.5b | ~1GB | 4GB | 轻量任务、测试 |
| qwen2.5:7b | ~5GB | 8GB | 日常使用 |
| llama3.2:3b | ~2GB | 4GB | 英文任务 |
| qwen2.5:14b | ~10GB | 16GB | 复杂任务 |

> 注意：本地模型能力不如 GPT-4，但对于学习和简单任务足够了！`, 'theory', 25, 'https://www.bilibili.com/video/BV1XiTkzVE4X/', 4, JSON.stringify([
    {title: 'Ollama 官网', url: 'https://ollama.com/'},
    {title: 'Ollama GitHub', url: 'https://github.com/ollama/ollama'},
    {title: 'Qwen 模型', url: 'https://ollama.com/library/qwen2.5'},
    {title: 'Llama 模型库', url: 'https://ollama.com/library/llama3.2'}
  ]))
    // 课程 2: Agent 基础概念
    insertCourse.run(2, 'Agent 基础概念与架构', 'basics', 'beginner', '了解 AI Agent 的核心概念、组成部分和工作原理', 2)
    insertChapter.run(5, 2, '什么是 AI Agent', `# 什么是 AI Agent
...
> 推荐进一步阅读：[Anthropic - Building effective agents](https://www.anthropic.com/research/building-effective-agents)`, 'theory', 15, 'https://www.bilibili.com/video/BV1XiTkzVE4X/', 1, JSON.stringify([
    ...
    insertChapter.run(6, 2, 'Agent 的核心架构', `# Agent 的核心架构
...
> 推荐阅读：[Lilian Weng - LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/)`, 'theory', 15, 'https://www.bilibili.com/video/BV1XiTkzVE4X/?p=2', 2, JSON.stringify([
    ...
    insertChapter.run(7, 2, '构建你的第一个 Agent', `# 构建你的第一个 Agent
...
> 推荐阅读：[OpenAI - Building agents with the OpenAI API](https://platform.openai.com/docs/guides/agents)`, 'theory', 15, 'https://www.bilibili.com/video/BV1XiTkzVE4X/?p=3', 3, JSON.stringify([
    ...
    // 课程 3: Prompt Engineering
    insertCourse.run(3, 'Prompt Engineering 进阶', 'basics', 'beginner', '掌握编写高质量 Prompt 的技巧，让 Agent 更精准地理解你的意图', 3)
    insertChapter.run(8, 3, 'Prompt 基础原则', `# Prompt 基础原则
...
> 推荐阅读：[OpenAI - Prompt engineering guide](https://platform.openai.com/docs/guides/prompt-engineering)`, 'theory', 15, 'https://www.bilibili.com/video/BV1Z7ZwYHENT/', 1, JSON.stringify([
    ...
    insertChapter.run(9, 3, '高级 Prompt 技巧', `# 高级 Prompt 技巧
...
> 推荐阅读：[Prompt Engineering Guide - 高级技巧](https://www.promptingguide.ai/zh/techniques/cot)`, 'theory', 15, 'https://www.bilibili.com/video/BV1Z7ZwYHENT/?p=2', 2, JSON.stringify([
    ...
    insertChapter.run(10, 3, 'Prompt 实战练习', `# Prompt 实战练习
...
> 推荐阅读：[Anthropic - Prompt engineering with Claude](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview)`, 'practice', 20, 'https://www.bilibili.com/video/BV1Z7ZwYHENT/?p=3', 3, JSON.stringify([
    ...
    // 课程 4: Tool Calling
    insertCourse.run(4, 'Tool Calling 与 Function Calling', 'agent', 'intermediate', '深入理解 Agent 如何调用工具，掌握 Function Calling 的核心机制', 4)
    insertChapter.run(11, 4, 'Tool Calling 概述', `# Tool Calling 概述
...
> 推荐阅读：[OpenAI - Function calling guide](https://platform.openai.com/docs/guides/function-calling)`, 'theory', 15, 'https://www.bilibili.com/video/BV1z95LzaE39/', 1, JSON.stringify([
    ...
    insertChapter.run(12, 4, '实现 Tool Calling', `# 实现 Tool Calling
...
> 推荐阅读：[LangChain - Tools and ToolCalling](https://python.langchain.com/docs/modules/agents/tools/)`, 'theory', 15, 'https://www.bilibili.com/video/BV1z95LzaE39/?p=2', 2, JSON.stringify([
    ...
    insertChapter.run(13, 4, 'Tool Calling 实战', `# Tool Calling 实战
...
> 推荐阅读：[LangChain - Custom Tools](https://python.langchain.com/docs/modules/agents/tools/custom_tools)`, 'practice', 20, 'https://www.bilibili.com/video/BV1z95LzaE39/?p=3', 3, JSON.stringify([
    ...
    // 课程 5: Agent 工作流
    insertCourse.run(5, 'Agent 工作流设计', 'agent', 'intermediate', '学习 Plan-Execute-Review 工作流模式，设计高效的 Agent 执行流程', 5)
    insertChapter.run(14, 5, 'Plan-Execute-Review 模式', `# Plan-Execute-Review 模式
...
> 推荐阅读：[Anthropic - Building effective agents](https://www.anthropic.com/research/building-effective-agents)`, 'theory', 15, 'https://www.bilibili.com/video/BV1mXZyYtEZS/', 1, JSON.stringify([
    ...
    insertChapter.run(15, 5, '工作流实战', `# 工作流实战
...
> 推荐阅读：[LangGraph - Build Stateful Agents](https://langchain-ai.github.io/langgraph/)`, 'practice', 20, 'https://www.bilibili.com/video/BV1mXZyYtEZS/?p=2', 2, JSON.stringify([
    ...
    // 课程 6: 多 Agent 协作
    insertCourse.run(6, '多 Agent 协作模式', 'advanced', 'advanced', '探索多个 Agent 协同工作的模式与最佳实践', 6)
    insertChapter.run(16, 6, '多 Agent 协作概述', `# 多 Agent 协作概述
...
> 推荐阅读：[AutoGen - Multi-agent framework](https://microsoft.github.io/autogen/)`, 'theory', 15, 'https://www.bilibili.com/video/BV17XP5eBE15/', 1, JSON.stringify([
    ...
    insertChapter.run(17, 6, '多 Agent 实战', `# 多 Agent 实战
...
> 推荐阅读：[CrewAI - Multi-agent framework](https://docs.crewai.com/)`, 'practice', 20, 'https://www.bilibili.com/video/BV17XP5eBE15/?p=2', 2, JSON.stringify([
    ...
    // 课程 7: RAG Agent
    insertCourse.run(7, 'RAG 在 Agent 中的应用', 'advanced', 'advanced', '掌握检索增强生成技术，让 Agent 拥有强大的知识检索能力', 7)
    insertChapter.run(18, 7, 'RAG 基础', `# RAG 基础
...
> 推荐阅读：[Pinecone - What is RAG?](https://www.pinecone.io/learn/retrieval-augmented-generation/)`, 'theory', 15, 'https://www.bilibili.com/video/BV1c1oxYtEZM/', 1, JSON.stringify([
    ...
    insertChapter.run(19, 7, 'RAG 实战', `# RAG 实战
...
> 推荐阅读：[LangChain - RAG Tutorial](https://python.langchain.com/docs/tutorials/rag)`, 'practice', 20, 'https://www.bilibili.com/video/BV1c1oxYtEZM/?p=2', 2, JSON.stringify([
    ...
    // ===== 引导任务 1：获取并配置你的第一个 API Key（入门）=====
    insertTask.run(
      1, 1, '获取并配置你的第一个 API Key',
      '在本任务中，你将去 OpenAI 或 DeepSeek 平台注册账号、获取 API Key，并配置环境变量。完成后你将拥有一个可用的 API Key，为后续章节做准备。',
      '// 本任务无需编写代码\n// 按照步骤注册账号、获取 API Key、配置环境变量即可',
      '',
      'guide', '', 'easy', 1,
      JSON.stringify([
        { title: '注册平台账号', description: '访问 OpenAI 或 DeepSeek 注册账号', hint: '推荐 DeepSeek，国内可访问', referenceCode: '// 访问 https://platform.deepseek.com/\n// 点击注册，使用手机号或邮箱', expectedResult: '成功注册并登录平台' },
        { title: '创建 API Key', description: '在平台 API Keys 页面创建新密钥', hint: '左侧菜单 → API Keys → 创建', referenceCode: '// 进入 API Keys 页面\n// 点击"创建 API Key"\n// 复制生成的密钥（sk-xxx）', expectedResult: '获得一个以 sk- 开头的 API Key' },
        { title: '配置环境变量', description: '将 API Key 配置为环境变量', hint: 'Windows 用 SetEnvironmentVariable，Mac/Linux 用 export', referenceCode: '# Windows PowerShell:\n[System.Environment]::SetEnvironmentVariable("OPENAI_API_KEY", "sk-xxx", "User")\n# Mac/Linux:\nexport OPENAI_API_KEY="sk-xxx"', expectedResult: '新终端中可读取到 OPENAI_API_KEY 环境变量' }
      ]),
      '[]',
      '',
      '本任务无需编程，关键是将 API Key 配置为环境变量。',
      0
    )
    // ===== 引导任务 2：用 Python 创建你的第一个 LangChain Agent（进阶）=====
    insertTask.run(
      2, 2, '用 Python 创建你的第一个 LangChain Agent',
      '在本任务中，你将使用 Python + LangChain 创建一个简单的对话 Agent。按照步骤安装依赖、编写代码，最终运行一个能对话的 Agent。',
      '# 安装依赖：pip install langchain langchain-openai python-dotenv\n# from langchain_openai import ChatOpenAI\n# from dotenv import load_dotenv\n# load_dotenv()\n# llm = ChatOpenAI(model="gpt-4o-mini")\n# response = llm.invoke("你好")\n# print(response.content)',
      '',
      'guide', '', 'medium', 2,
      JSON.stringify([
        { title: '安装依赖', description: '创建项目目录，安装 LangChain 和 OpenAI 包', hint: 'pip install langchain langchain-openai python-dotenv', referenceCode: 'mkdir my-agent && cd my-agent\npython -m venv venv\nsource venv/bin/activate\npip install langchain langchain-openai python-dotenv', expectedResult: '成功安装 langchain 包' },
        { title: '创建 LLM 实例', description: '编写代码创建 ChatOpenAI 实例并测试', hint: '使用 ChatOpenAI(model="gpt-4o-mini")', referenceCode: 'from langchain_openai import ChatOpenAI\nfrom dotenv import load_dotenv\nload_dotenv()\nllm = ChatOpenAI(model="gpt-4o-mini", temperature=0.7)\nresponse = llm.invoke("你好")\nprint(response.content)', expectedResult: '运行后能看到 AI 回复' },
        { title: '构建 Agent', description: '使用 create_tool_calling_agent 创建 Agent', hint: '需要定义工具、创建 prompt、调用 create_tool_calling_agent', referenceCode: 'from langchain.agents import create_tool_calling_agent, AgentExecutor\nfrom langchain_core.prompts import ChatPromptTemplate\nfrom langchain_core.tools import tool\n\n@tool\ndef calculate(expr: str) -> str:\n    """计算数学表达式"""\n    return str(eval(expr))\n\ntools = [calculate]\nprompt = ChatPromptTemplate.from_messages([\n    ("system", "你是一个有用的助手。"),\n    ("user", "{input}"),\n    ("placeholder", "{agent_scratchpad}"),\n])\nagent = create_tool_calling_agent(llm, tools, prompt)\nexecutor = AgentExecutor(agent=agent, tools=tools)', expectedResult: 'Agent 执行器创建成功' },
        { title: '运行对话', description: '运行 Agent 进行对话测试', hint: 'executor.invoke({"input": "你的问题"})', referenceCode: 'result = executor.invoke({"input": "请计算 123 * 456"})\nprint(result["output"])', expectedResult: 'Agent 正确调用工具并返回结果' }
      ]),
      '[]',
      '',
      '关键是三步：创建 LLM → 定义工具 → 构建 Agent。不要忘记 load_dotenv()。',
      0
    )
    // ===== 引导任务 3：为 Agent 添加工具调用能力（进阶）=====
    insertTask.run(
      3, 2, '为 Agent 添加工具调用能力',
      '在本任务中，你将给 LangChain Agent 添加一个简单工具（如计算器或天气查询），让 Agent 能够调用外部工具完成任务。',
      '# 在任务2的基础上添加更多工具\n# from langchain_core.tools import tool\n#\n# @tool\n# def search_weather(city: str) -> str:\n#     """查询城市天气"""\n#     return f"{city}今天晴，25度"\n#\n# tools = [calculate, search_weather]\n# agent = create_tool_calling_agent(llm, tools, prompt)',
      '',
      'guide', '', 'medium', 3,
      JSON.stringify([
        { title: '定义工具', description: '使用 @tool 装饰器定义一个新工具函数', hint: '工具函数需要有 docstring 描述功能，Agent 会根据描述决定是否调用', referenceCode: 'from langchain_core.tools import tool\n\n@tool\ndef search_weather(city: str) -> str:\n    """查询指定城市的天气"""\n    # 这里模拟天气查询\n    return f"{city}今天晴，25度"', expectedResult: '工具函数定义成功，有 docstring 描述' },
        { title: '绑定工具到 Agent', description: '将新工具添加到 tools 列表，重新创建 Agent', hint: 'tools = [calculate, search_weather]，然后重新调用 create_tool_calling_agent', referenceCode: 'tools = [calculate, search_weather]\nagent = create_tool_calling_agent(llm, tools, prompt)\nexecutor = AgentExecutor(agent=agent, tools=tools, verbose=True)', expectedResult: 'Agent 创建成功，绑定了两个工具' },
        { title: '测试工具调用', description: '让 Agent 处理需要调用工具的问题', hint: '问"北京天气怎么样"，Agent 应该调用 search_weather 工具', referenceCode: 'result = executor.invoke({"input": "北京天气怎么样？"})\nprint(result["output"])', expectedResult: 'Agent 调用 search_weather 工具并返回天气信息' },
        { title: '处理工具结果', description: '验证 Agent 能正确处理工具返回的结果并生成最终回复', hint: '尝试多个问题，确保 Agent 选择正确的工具', referenceCode: '# 测试不同问题\nresult1 = executor.invoke({"input": "计算 99 * 88"})\nprint(result1["output"])\nresult2 = executor.invoke({"input": "上海天气如何"})\nprint(result2["output"])', expectedResult: 'Agent 能根据问题选择正确的工具并返回正确结果' }
      ]),
      '[]',
      '',
      '工具的核心是 @tool 装饰器和 docstring。Agent 通过 docstring 理解工具功能并决定何时调用。',
      0
    )
    // ===== 引导任务 4：用 Node.js 实现流式输出 Agent（进阶）=====
    insertTask.run(
      4, 3, '用 Node.js 实现流式输出 Agent',
      '在本任务中，你将用 Node.js + OpenAI SDK 实现一个支持流式输出的 Agent，让用户能看到逐字输出的效果。',
      '# npm init -y && npm install openai dotenv\n# 在 package.json 中添加 "type": "module"\n# import OpenAI from "openai"\n# import "dotenv/config"\n# const client = new OpenAI()\n# const stream = await client.chat.completions.create({\n#   model: "gpt-4o-mini",\n#   messages: [{ role: "user", content: "你好" }],\n#   stream: true,\n# })\n# for await (const chunk of stream) {\n#   process.stdout.write(chunk.choices[0]?.delta?.content || "")\n# }',
      '',
      'guide', '', 'medium', 4,
      JSON.stringify([
        { title: '初始化项目', description: '创建 Node.js 项目并安装依赖', hint: 'npm init -y && npm install openai dotenv', referenceCode: 'mkdir node-agent && cd node-agent\nnpm init -y\nnpm install openai dotenv\n# 在 package.json 添加 "type": "module"', expectedResult: '项目创建成功，依赖安装完成' },
        { title: '编写流式调用代码', description: '使用 OpenAI SDK 的 stream 参数实现流式输出', hint: '设置 stream: true，用 for await 遍历响应', referenceCode: 'import OpenAI from "openai"\nimport "dotenv/config"\n\nconst client = new OpenAI()\nconst stream = await client.chat.completions.create({\n  model: "gpt-4o-mini",\n  messages: [{ role: "user", content: "讲一个故事" }],\n  stream: true,\n})\nfor await (const chunk of stream) {\n  process.stdout.write(chunk.choices[0]?.delta?.content || "")\n}', expectedResult: '运行后能看到逐字输出的效果' },
        { title: '处理流式响应', description: '将流式输出封装成可复用的函数', hint: '创建 async function streamChat(message) 封装流式逻辑', referenceCode: 'async function streamChat(message) {\n  const stream = await client.chat.completions.create({\n    model: "gpt-4o-mini",\n    messages: [{ role: "user", content: message }],\n    stream: true,\n  })\n  let fullResponse = ""\n  for await (const chunk of stream) {\n    const content = chunk.choices[0]?.delta?.content || ""\n    process.stdout.write(content)\n    fullResponse += content\n  }\n  return fullResponse\n}', expectedResult: '调用 streamChat 函数能流式输出并返回完整响应' },
        { title: '组合成完整 Agent', description: '添加交互式命令行，实现可对话的流式 Agent', hint: '使用 readline 或 while 循环读取用户输入', referenceCode: 'import * as readline from "readline"\n\nconst rl = readline.createInterface({ input: process.stdin, output: process.stdout })\n\nfunction ask() {\n  rl.question("你：", async (input) => {\n    if (input === "exit") { rl.close(); return }\n    process.stdout.write("Agent：")\n    await streamChat(input)\n    console.log()\n    ask()\n  })\n}\nask()', expectedResult: '可以在命令行中与 Agent 进行流式对话' }
      ]),
      '[]',
      '',
      '流式输出的关键是 stream: true 参数和 for await 循环。封装成函数后可以复用。',
      0
    )
    // ===== 引导任务 5：用 Ollama 部署本地 Agent（高级）=====
    insertTask.run(
      5, 4, '用 Ollama 部署本地 Agent',
      '在本任务中，你将安装 Ollama、拉取开源模型，并配置 LangChain 使用本地模型而非云端 API 来运行 Agent。',
      '# 安装 Ollama：访问 https://ollama.com 下载\n# 拉取模型：ollama pull qwen2.5:7b\n# Python 代码：\n# from langchain_openai import ChatOpenAI\n# llm = ChatOpenAI(\n#     base_url="http://localhost:11434/v1",\n#     api_key="ollama",\n#     model="qwen2.5:7b",\n# )',
      '',
      'guide', '', 'hard', 5,
      JSON.stringify([
        { title: '安装 Ollama', description: '从官网下载并安装 Ollama', hint: '访问 https://ollama.com 下载对应系统的安装包', referenceCode: '# Windows/Mac: 下载安装包运行\n# Linux: curl -fsSL https://ollama.com/install.sh | sh\n# 验证：ollama --version', expectedResult: '终端能显示 Ollama 版本号' },
        { title: '拉取模型', description: '使用 ollama pull 拉取一个开源模型', hint: '推荐 qwen2.5:7b，中文能力好。配置低的电脑用 qwen2.5:1.5b', referenceCode: 'ollama pull qwen2.5:7b\n# 查看已安装模型\nollama list\n# 测试运行\nollama run qwen2.5:7b "你好"', expectedResult: '模型下载完成，能通过 ollama run 运行' },
        { title: '配置 API 端点', description: '验证 Ollama 的 OpenAI 兼容 API 端点可用', hint: 'Ollama 默认在 http://localhost:11434 提供 API', referenceCode: '# 测试 API\ncurl http://localhost:11434/v1/chat/completions -H "Content-Type: application/json" -d \'{"model":"qwen2.5:7b","messages":[{"role":"user","content":"你好"}]}\'', expectedResult: '能收到 JSON 格式的响应' },
        { title: '创建本地 Agent', description: '使用 LangChain 连接本地 Ollama 模型创建 Agent', hint: '只需把 base_url 改为 http://localhost:11434/v1，api_key 随便填', referenceCode: 'from langchain_openai import ChatOpenAI\nfrom langchain.agents import create_tool_calling_agent, AgentExecutor\nfrom langchain_core.prompts import ChatPromptTemplate\nfrom langchain_core.tools import tool\n\n@tool\ndef calculate(expr: str) -> str:\n    """计算数学表达式"""\n    return str(eval(expr))\n\ntools = [calculate]\nllm = ChatOpenAI(\n    base_url="http://localhost:11434/v1",\n    api_key="ollama",\n    model="qwen2.5:7b",\n)\nprompt = ChatPromptTemplate.from_messages([\n    ("system", "你是一个有用的助手。"),\n    ("user", "{input}"),\n    ("placeholder", "{agent_scratchpad}"),\n])\nagent = create_tool_calling_agent(llm, tools, prompt)\nexecutor = AgentExecutor(agent=agent, tools=tools)\nresult = executor.invoke({"input": "计算 123 * 456"})\nprint(result["output"])', expectedResult: 'Agent 使用本地模型成功运行并返回结果' }
      ]),
      '[]',
      '',
      'Ollama 兼容 OpenAI API 格式，只需修改 base_url 和 api_key。模型大小取决于你的电脑内存。',
      0
    )
    // ===== 引导任务 6：综合实战：构建多轮对话 Agent（高级引导项目）=====
    insertTask.run(
      6, null, '综合实战：构建多轮对话 Agent',
      '在本任务中，你将综合所学，构建一个支持多轮对话、上下文记忆、工具调用的完整 Agent。\n\n**这是一个引导式综合项目**，请按照步骤逐步完成。完成后你将拥有一个功能完整的对话 Agent！\n\n**要求**：\n- Agent 能记住之前的对话内容\n- Agent 能调用工具（如计算器、天气查询）\n- Agent 能进行多轮连续对话',
      '# 综合实战：构建多轮对话 Agent\n# 需要：LangChain + 记忆 + 工具\n#\n# from langchain_openai import ChatOpenAI\n# from langchain.agents import create_tool_calling_agent, AgentExecutor\n# from langchain_core.prompts import ChatPromptTemplate\n# from langchain_core.tools import tool\n# from langchain.memory import ConversationBufferMemory\n#\n# # 1. 定义工具\n# # 2. 创建带记忆的 prompt\n# # 3. 构建 Agent\n# # 4. 实现多轮对话循环',
      '',
      'guide', '', 'hard', 6,
      JSON.stringify([
        { title: '设计 Agent 架构', description: '规划 Agent 的组件：LLM、工具、记忆、对话循环', hint: '需要：ChatOpenAI + tools + ConversationBufferMemory + 交互循环', referenceCode: '# 架构设计\n# LLM: ChatOpenAI(model="gpt-4o-mini")\n# 工具: calculate, search_weather\n# 记忆: ConversationBufferMemory\n# 对话: while True 循环\nprint("Agent 架构：LLM + Tools + Memory + Loop")', expectedResult: '完成架构设计，明确各组件职责' },
        { title: '实现对话记忆', description: '使用 ConversationBufferMemory 让 Agent 记住对话历史', hint: '在 prompt 中加入 {history} 占位符，配置 ConversationBufferMemory', referenceCode: 'from langchain.memory import ConversationBufferMemory\nfrom langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder\n\nmemory = ConversationBufferMemory(memory_key="history", return_messages=True)\nprompt = ChatPromptTemplate.from_messages([\n    ("system", "你是一个智能助手，能记住对话内容。"),\n    MessagesPlaceholder(variable_name="history"),\n    ("user", "{input}"),\n    ("placeholder", "{agent_scratchpad}"),\n])', expectedResult: '记忆组件创建成功，prompt 包含 history 占位符' },
        { title: '集成工具调用', description: '将工具和记忆集成到 Agent 中', hint: '使用 create_tool_calling_agent 并传入 tools 和 prompt', referenceCode: 'from langchain_openai import ChatOpenAI\nfrom langchain.agents import create_tool_calling_agent, AgentExecutor\nfrom langchain_core.tools import tool\n\n@tool\ndef calculate(expr: str) -> str:\n    """计算数学表达式"""\n    return str(eval(expr))\n\n@tool\ndef search_weather(city: str) -> str:\n    """查询天气"""\n    return f"{city}今天晴，25度"\n\ntools = [calculate, search_weather]\nllm = ChatOpenAI(model="gpt-4o-mini", temperature=0)\nagent = create_tool_calling_agent(llm, tools, prompt)\nexecutor = AgentExecutor(agent=agent, tools=tools, memory=memory)', expectedResult: 'Agent 创建成功，包含工具和记忆' },
        { title: '测试多轮对话', description: '运行多轮对话测试，验证记忆和工具调用', hint: '先问"我叫小明"，再问"我叫什么"，最后问"计算 1+1"', referenceCode: '# 测试多轮对话\nresult1 = executor.invoke({"input": "我叫小明，今年25岁"})\nprint(result1["output"])\n\nresult2 = executor.invoke({"input": "我叫什么名字？"})\nprint(result2["output"])  # 应该回答"小明"\n\nresult3 = executor.invoke({"input": "北京天气怎么样？"})\nprint(result3["output"])  # 应该调用天气工具\n\nresult4 = executor.invoke({"input": "计算我的年龄乘以10"})\nprint(result4["output"])  # 应该调用计算工具，结果250', expectedResult: 'Agent 能记住"小明"，能调用天气和计算工具' }
      ]),
      '[]',
      '',
      '这是综合项目，关键是将记忆、工具和对话循环组合起来。记忆让 Agent 有上下文，工具让 Agent 有能力。',
      1
    )
    // 实操任务（6 个引导任务 + 5 个核心测试任务 + 1 个综合实战引导项目）
    // 引导任务（1-6）使用 'guide' 验证类型，无需编写代码；测试任务（7-12）使用 'tests' 验证类型，通过 Web Worker 执行用户代码并运行单元测试
    ...
    // ===== 任务 12：综合实战引导项目 - Mini Agent =====
    insertTask.run(
      12, null, '综合实战：构建 Mini Agent',
