...
-   **7 门系统课程** — 部署自己的 Agent、Agent 基础、Prompt Engineering、Tool Calling、工作流设计、多 Agent 协作、RAG 技术
-   **19 个章节** — 理论 + 实操，每章配有真实 Bilibili 教学视频 + 4 个配套学习资源链接
-   **12 个实操任务** — 6 道 guide 实践题（引导模式，无在线测试）+ 6 个 tests 任务（含综合实战引导项目），内置 Monaco 代码编辑器，分步引导（含预期效果+参考代码）+ 单元测试验证
...
> **v1.3.0 更新**：新增课程1「部署自己的 Agent」（4 章节：环境准备与 API Key 配置、Python+LangChain 部署 Agent、Node.js 部署 Agent、本地部署开源模型 Ollama），现有 6 门课程顺延为课程 2-7；实操实验室新增 6 道 guide 模式实践题（获取并配置 API Key、Python LangChain Agent、Agent 工具调用、Node.js 流式输出、Ollama 本地 Agent、综合实战多轮对话 Agent），仅提供题目描述与分步引导，用户在自己的 Agent 环境完成并手动标记完成，现有 6 道任务顺延为任务 7-12；validationType 新增 guide 纯引导类型。总计 7 课程 / 19 章节 / 12 任务（6 guide + 6 tests）。
>
> **v1.2.2 更新**：修复章节视频不显示的前端字段映射 bug — `loadChapters` 未将数据库 `video_url` 字段映射为前端 `videoUrl`，导致 B 站教学视频区域始终不渲染
>
> **v1.2.1 更新**：修复 v1.2.0 升级时数据库种子数据不刷新的严重 bug — 新增 `app_meta` 版本表，升级时自动检测数据版本并清除旧课程/章节/任务数据重新填充，确保视频、资源、任务引导正确生效
>
...
1. **下载** — 前往 [Releases 页面](https://github.com/lbh1nb/ai-agent-learner/releases)，下载最新版的 `AI Coding Learner-1.3.0-win.zip`
2. **解压** — 将 zip 文件解压到任意目录（如桌面）
3. **运行** — 双击解压目录中的 `AI Coding Learner.exe`

> 也可以下载 `AI Coding Learner 1.3.0.exe` 便携版，双击直接运行，无需解压。
...
| 课程学习 | 浏览全部 7 门课程，按分类筛选，进入课程详情 |
| 实操实验室 | 12 个任务（6 guide 实践题 + 6 tests 任务），Monaco 编辑器 + 预期效果 + 参考代码 + 单元测试 + AI 助手 |
...
5. **实操练习** — 进入「实操实验室」，tests 任务按分步引导（含预期效果 + 参考代码）在 Monaco 编辑器中编写代码并运行测试用例验证；guide 实践题按分步引导在自己的 Agent 环境完成后手动标记完成，与 AI 助手对话答疑
...
打包后会在 `release/` 目录生成：
- `AI Coding Learner 1.3.0.exe` — 便携版，双击即运行
- `AI Coding Learner-1.3.0-win.zip` — 解压版，解压后运行
...
│   ├── database.ts      # SQLite 数据库初始化与种子数据（含 19 章节 B 站视频 + 资源 + 12 任务分步引导）
