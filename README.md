...
-   **6 个实操任务** — 5 个核心任务 + 1 个综合实战引导项目，内置 Monaco 代码编辑器，分步引导 + 单元测试验证
...
1. **下载** — 前往 [Releases 页面](https://github.com/lbh1nb/ai-agent-learner/releases)，下载最新版的 `AI Coding Learner-1.1.0-win.zip`
2. **解压** — 将 zip 文件解压到任意目录（如桌面）
3. **运行** — 双击解压目录中的 `AI Coding Learner.exe`

> 也可以下载 `AI Coding Learner 1.1.0.exe` 便携版，双击直接运行，无需解压。
...
| 实操实验室 | 6 个分步引导任务（含引导项目），Monaco 编辑器 + 单元测试 + AI 助手 |
...
5. **实操练习** — 进入「实操实验室」，按分步引导在 Monaco 编辑器中编写代码，运行测试用例验证，与 AI 助手对话答疑
...
打包后会在 `release/` 目录生成：
- `AI Coding Learner 1.1.0.exe` — 便携版，双击即运行
- `AI Coding Learner-1.1.0-win.zip` — 解压版，解压后运行
...
│   ├── database.ts      # SQLite 数据库初始化与种子数据
